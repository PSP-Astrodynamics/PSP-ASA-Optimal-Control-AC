...
function [prob_6DoF] = MPC_6DoF_reynolds_quat_func(ref, t_horizon, Q, R, x_0, u_0, Isp, I)
arguments
    ref
    t_horizon
    Q
...
%% Specify Objective
tracking_objective = @(x, u, p, k) einsum(@(k) quad_form(u(4, k) - u_ref_sol(1:3, k), R(1:3, 1:3)) + Q(4, 4) * abs(u(4, k) - u_ref_sol(4, k)), 1:N) + quad_form(x(:, N) - x_ref_sol(:, N), Q);
...
prob_6DoF = DeterministicProblem(x_0, x_f, N, u_hold, tf, f, guess, convex_constraints, tracking_objective, scale = scale, scale_hint = scale_hint, terminal_bc = terminal_bc, nonconvex_constraints = nonconvex_constraints, discretization_method = "error", N_sub = 1);
end
% 
% %%
